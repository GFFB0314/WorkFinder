...
export function EditWatchlistModal({ isOpen, onClose, onSuccess, watchlist }: EditWatchlistModalProps) {
    ...
    // Pre-fill form with existing watchlist data
    useEffect(() => {
        if (watchlist) {
            setKeywords(watchlist.keywords.join(", "));
            setLocation(watchlist.location || "");
            setRemoteOnly(watchlist.remote_only);
            setExperienceLevel(watchlist.experience_level || "");
            setMinSalary(watchlist.min_salary?.toString() || "");
            setMaxSalary(watchlist.max_salary?.toString() || "");
            setFrequency(watchlist.frequency);
            setActive(watchlist.active);
        }
    }, [watchlist]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await api.put(`/watchlists/${watchlist.id}`, watchlistData);
            onSuccess();
            onClose();
        } catch (err: any) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 py-8 px-4 overflow-y-auto">
            <div className="relative w-full max-w-2xl my-auto rounded-xl border border-slate-200 bg-white p-8 shadow-lg">
                ...
                <h2 className="mb-6 text-2xl font-bold text-slate-900">Edit Watchlist</h2>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        />
                        <label htmlFor="editRemoteOnly" className="text-sm font-medium text-slate-700">
                            Remote jobs only
                        </label>
                    </div>
