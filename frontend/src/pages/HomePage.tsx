...
export function HomePage() {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/20 to-indigo-50/10 font-sans text-slate-900">
            <Header />

            <main>
                {/* Hero Section */}
                <section className="relative overflow-hidden bg-gradient-to-br from-white via-blue-50/40 to-indigo-50/30 px-4 py-20 sm:py-28 text-center">
                    {/* Animated background orbs */}
                    <motion.div
                        className="absolute -top-40 -right-40 h-80 w-80 rounded-full bg-blue-200/30 blur-3xl"
                        animate={{ x: [0, 30, 0], y: [0, -20, 0], scale: [1, 1.1, 1] }}
                        transition={{ duration: 8, repeat: Infinity, ease: "easeInOut" }}
                    />
                    <motion.div
                        className="absolute -bottom-32 -left-32 h-72 w-72 rounded-full bg-indigo-200/30 blur-3xl"
                        animate={{ x: [0, -20, 0], y: [0, 30, 0], scale: [1, 1.15, 1] }}
                        transition={{ duration: 10, repeat: Infinity, ease: "easeInOut" }}
                    />
                    <motion.div
                        className="absolute top-20 left-1/2 h-60 w-60 rounded-full bg-violet-200/20 blur-3xl"
                    ...
                    <div className="container relative mx-auto max-w-4xl">
                        ...
                        <motion.div
                            ...
                            className="mb-6 inline-flex items-center gap-2 rounded-full bg-blue-50 border border-blue-100 px-4 py-1.5 text-sm font-medium text-blue-700"
                        >
                            <Sparkles className="h-4 w-4" />
                            Aggregating jobs from 4+ sources
                        </motion.div>
                        ...
                        <motion.p
                            ...
                            className="mb-10 text-lg text-slate-600 max-w-2xl mx-auto"
                        >
                            Aggregating the best developer jobs from Remotive, Adzuna, and more.
                            Set up watchlists and get notified when your dream job appears.
                        </motion.p>
                        ...
                        <motion.form
                            ...
                            className="mx-auto flex max-w-2xl flex-col gap-4 rounded-2xl bg-white/90 backdrop-blur-md p-2.5 shadow-xl shadow-blue-900/5 ring-1 ring-slate-200/80 sm:flex-row sm:items-center"
                        >
                            <div className="flex flex-1 items-center gap-2 px-3">
                                <Search className="h-5 w-5 text-slate-400" />
                                <Input
                                    placeholder="Python, React, Engineer..."
                ...
                <section className="container mx-auto flex flex-col gap-8 px-4 py-12 lg:flex-row">
                    ...
                    >
                        <div className="sticky top-24 rounded-2xl border border-slate-200/80 bg-white/80 backdrop-blur-sm p-6 shadow-sm">
                            ...
                            <div className="space-y-6">
                                <div>
                                    <h4 className="mb-3 text-sm font-medium text-slate-700">Work Type</h4>
                                    <label className="flex items-center gap-2 text-sm text-slate-600 cursor-pointer group">
                                        ...
                                        />
                                        <span className="group-hover:text-blue-600 transition-colors">Remote Only</span>
                                    </label>
                                </div>
                                <div>
                                    <h4 className="mb-3 text-sm font-medium text-slate-700">Experience / Skills</h4>
                                    <div className="space-y-2">
                                        {["Junior", "Mid", "Senior", "Lead"].map((level) => (
                                            <label key={level} className="flex items-center gap-2 text-sm text-slate-600 cursor-pointer group">
                                                <input
                                                    type="checkbox"
                                                    ...
                                                    className="rounded border-slate-300 text-blue-600 focus:ring-blue-600"
                                                />
                                                <span className="group-hover:text-blue-600 transition-colors">{level}</span>
                                            </label>
                                        ))}
                                    </div>
                                </div>
                    ...
                    <div className="flex-1">
                        ...
                        >
                            ...
                            <div className="flex items-center gap-2">
                                <span className="text-sm text-slate-500">Auto-updates as you type</span>
                            </div>
                        </motion.div>
                                    ...
                                    {jobs.length > 0 ? (
                                        ...
                                        >
                                            ...
                                            <p className="text-slate-500 mt-1">
                                                Try adjusting your search for "{search}" {location && `in ${location}`}
                                            </p>
                                        </motion.div>
                                    )}
                                </motion.div>
